...
float ramp(float current, float target, float factor) {
  ...
  while (1) {
    if (ps5.isConnected()) {
      ...
      if (start % 2 == 1) {
        ...
        // curr_vx = ramp(curr_vx, (float)vx, accel) ;
        // curr_vy = ramp(curr_vy, (float)vy, accel) ;
        // curr_wz = (float)wz;
...
void cmode1() {
  if (ps5.R2Value() > 30 || ps5.L2Value() > 30 ||
    ...
    if (ps5.R2Value() > 30 || ps5.L2Value() > 30) {
      wz = map(ps5.R2Value() - ps5.L2Value(), -255, 255, -70, 70);
      c_set = input_angle;
    } else {
      wz = 0;
    }
