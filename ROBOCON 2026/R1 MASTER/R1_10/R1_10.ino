  ...
  while (1) {
    if (ps5.isConnected()) {
      ...
      if (start % 2 == 1) {
        ...
        c_s = constrain(c_s, -255, 255);

        ps = ps5.RStickY();

        (ch_mode % 2 == 0) ? c_s = spd : c_s = spd2;

        if (ps5.R2Value() > 30 || ps5.L2Value() > 30 || ps5.Right() || ps5.Left() || ps5.Up() || ps5.Down()) {
          if (ps5.R2Value() > 30 || ps5.L2Value() > 30) {
            wz = map(ps5.R2Value() - ps5.L2Value(), -255, 255, -(c_s + 20), (c_s - 20));
            prev_wz = wz;
            braking = false;
            is_rotating = true;
            was_rotating = true;
          ...
          if (ps5.Up()) vx = -c_s;
          else if (ps5.Down()) vx = c_s;
          else vx = 0;
          if (ps5.Right()) vy = -c_s;
          else if (ps5.Left()) vy = c_s;
          else vy = 0;
