...
volatile bool was_rotating = false;

float c_kp = 4, c_ki = 0.003, c_kd = 3.0;//====== PID VALUES =========
...
void cmode1(){
  ...
  curr_wz = acceleration(curr_wz, (float)wz, 0.15);
}
void cmode2(){
...
int pid_calc(float kp, float ki, float kd, int av, int sp) {
  ...
  int Speed = max(abs(vx), abs(vy));

  float scale = 1.0 - ((float)Speed / 255.0) * 0.4;
  ...
  // int maxCorrection = map(Speed, 0, 255, 120, 55);
    int maxCorrection = map(Speed, 0, 255, 150, 90);
