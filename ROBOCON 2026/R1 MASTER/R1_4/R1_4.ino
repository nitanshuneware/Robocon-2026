    ...
    if (ps5.isConnected() == true) {
      ...
      if(ps5.PSButton()){
        ...
        if(start%2 == 1){
          ...
          if (ps5.LStickY() > 50)  esc.writeMicroseconds(bldc_su + bldc_s);
          else if (ps5.LStickY() < -50) esc.writeMicroseconds(bldc_sd - bldc_s);
          else esc.writeMicroseconds(1500);

          if( mode % 2 == 0){
            if      ( ps5.RStickY() > 50 )    { espSerial.println("S_up");  Serial.println("S_up"); }
            else if ( ps5.RStickY() < -50 )   { espSerial.println("S_dn");  Serial.println("S_dn"); }
            else                              { espSerial.println("S_s");   Serial.println("S_stop"); }
            ps5.Cross()    ? espSerial.println("S1")   : espSerial.println("s1s");
            ps5.Circle()   ? espSerial.println("S2")   : espSerial.println("s2s");
            ps5.Triangle() ? espSerial.println("SH1")  : espSerial.println("sh1s");
            ps5.Square()   ? espSerial.println("SH2")  : espSerial.println("sh2s");
          }
          else{
            if      ( ps5.RStickY() > 50 )    { espSerial.println("KF_up"); Serial.println("KF_up"); }
            else if ( ps5.RStickY() < -50 )   { espSerial.println("KF_dn"); Serial.println("KF_dn"); }
            else                              { espSerial.println("KF_s");  Serial.println("KF_stop"); }
            ps5.Cross()    ? espSerial.println("KF_C")  : espSerial.println("KF_S");
            ps5.Circle()   ? espSerial.println("KF_AC") : espSerial.println("KF_S");
            ps5.Triangle() ? espSerial.println("KFH_F") : espSerial.println("KFH_S");
            ps5.Square()   ? espSerial.println("KFH_B") : espSerial.println("KFH_S");
          }
        }
        else{
