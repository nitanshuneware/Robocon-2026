  ...
  if (ps5.isConnected() == true) {
    ...
    if(ps5.PSButton()){
      ...
      if( start%2 == 1){
        ...
        if(cbi != pbi && cbi == 1)   bldc_s += 10 ; // bldc speed increment
        if(cbd != pbd && cbd == 1)   bldc_s -= 5 ; // bldc speed decrement
        pre_r = curr_r ;
        pre_l = curr_l ;
        pbi = cbi ;
        pbd = cbd ;
        ...
        else if (ps5.LStickY() > 50  )      {    esc.writeMicroseconds(bldc_su + bldc_s); Serial.print(bldc_s); }
        else if (ps5.LStickY() < -50  )     {    esc.writeMicroseconds(bldc_sd - bldc_s); }
        else if (ps5.RStickY() > 50  )      {    espSerial.println("Su"); }
        else if (ps5.RStickY() < -50  )     {    espSerial.println("Sd"); }
        else if (ps5.RStickY() > 50  )      {    espSerial.println("Su"); }
        else if (ps5.RStickY() < -50  )     {    espSerial.println("Sd"); }
        else stop();
        Serial.println();
